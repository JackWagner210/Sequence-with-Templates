...
namespace sequence = CS3358_FA2018_A04_sequence;
using namespace std;
...
template <class T>
void show_list(sequence::sequence <T>src);
// Pre: (none)
...
//       or real number.
//       The prompt is repeated until a non-whitespace character or
//       real number can be read. The non-whitespace character read
//       is returned. The input buffer is cleared of any extra input
//       until and including the first newline character.
int main(int argc, char *argv[])
{
   sequence::sequence <double>s1;  // A sequence of double for testing
   sequence::sequence <char>s2; // A sequence of char for testing
   ...
   do
   {
      ...
      switch (choice)
      {
         ...
         case 'A':
            ...
            if (objectNum == 1)
            {
               numHold = get_entry<int>();
               s1.add(numHold);
               cout << numHold << " added to s1." << endl;
            }
            else
            {
               charHold = get_entry<char>();
               s2.add(charHold);
...
{
   T result;
   cout << "Enter a real number or non-whitespace character: ";
   cin  >> result;
   while ( ! cin.good() )
   {
      cerr << "Invalid real number or non-whitespace character input..."
           << endl;
      cin.clear();
      cin.ignore(999, '\n');
      cout << "Re-enter a real number or non-whitespace character";
      cin  >> result;
   }
   // cin.ignore(999, '\n');
